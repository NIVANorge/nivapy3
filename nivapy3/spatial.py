...
def utm_to_wgs84_dd(utm_df, zone="utm_zone", east="utm_east", north="utm_north"):
    ...
    # Process per zone
    for z in df[zone].dropna().unique():
        mask = df[zone] == z
        epsg_utm = 25800 + int(z)
        transformer = Transformer.from_crs(
            f"epsg:{epsg_utm}", "epsg:4326", always_xy=True
        )
        valid = mask & df[east].notna() & df[north].notna()
        lon, lat = transformer.transform(
            df.loc[valid, east].values, df.loc[valid, north].values
        )
        lons[valid] = lon
        lats[valid] = lat
...
def wgs84_dd_to_utm(dd_df, lat="latitude", lon="longitude", utm_zone=33):
    ...
    Args:
        ...
        utm_zone:  Int. Desired UTM zone

    Returns:
        Copy of dd_df with 'utm_east', 'utm_north' and 'utm_zone' columns added.
    """
    df = dd_df.copy()

    easts = np.full(len(df), np.nan)
    norths = np.full(len(df), np.nan)

    transformer = Transformer.from_crs(
        "EPSG:4326", f"EPSG:{25800 + int(utm_zone)}", always_xy=True
    )

    valid = df[lat].notna() & df[lon].notna()

    east, north = transformer.transform(
        df.loc[valid, lon].values, df.loc[valid, lat].values
    )
